...
def plot_funding_rates(coin, time_frame):
    ...
    try:
        ...
        avg_funding_rate_binance = round(
            sum(funding_rates_binance) / len(funding_rates_binance))
    ...
    print([100 * 365 * 24 * float(item["fundingRate"]) for item in
             data_funding_binance])
    print(x)
    print(get_funding_rates_binance_period(pair))
    print(len(funding_rates_binance))
    print(len(funding_rates_hyper))
